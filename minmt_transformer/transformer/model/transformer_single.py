...
class Transformer_single(object):
    ...
    def __call__(self, inputs, scope="Transformer", targets=None):
        ...
        with tf.variable_scope(scope, initializer=initializer, reuse=tf.AUTO_REUSE):
            if self.mode == ModeKeys.PREDICT_ONE:
                ...
                if not self.params.share_encoder_decoder:
                    ...
                    if self.params.add_tag:

                        inputs1 = tf.constant(self.params.source_vocab_size, dtype=tf.int64)[None, None]
                        # Expand dims. Shape=(1, 1)
                        inputs1 = tf.tile(inputs1, [tf.shape(inputs)[0], 1])
                        inputs1 = tf.concat([inputs1, inputs, inputs1], 1)
                        if targets is not None:
                            targets1 = tf.constant(self.params.target_vocab_size, dtype=tf.int64)[None, None]
                            targets1 = tf.tile(targets1, [tf.shape(targets)[0], 1])
                            targets1 = tf.concat([targets1, targets, targets1], 1)
                            attention_bias2 = model_utils.get_padding_bias(targets1)
                    else:
                        inputs1 = inputs
                        if targets is not None:
                            targets1 = targets
                            attention_bias2 = model_utils.get_padding_bias(targets1)
                    attention_bias1 = model_utils.get_padding_bias(inputs1)
            ...
            if not self.params.share_encoder_decoder:
                ...
                if targets is not None:
                    encoder_outputs1 = self.encode(inputs1, attention_bias1)
                    encoder_outputs2 = self.encode(targets1, attention_bias2, self.params.share_encoder_decoder)
                    logits1 = self.decode(targets, encoder_outputs1, attention_bias1)
                    logits2 = self.decode(inputs, encoder_outputs2, attention_bias2, self.params.share_encoder_decoder)
                    return (logits1, logits2)
                else:
                    encoder_outputs = self.encode(inputs1, attention_bias1, self.params.predict_inverse)
                    return self.predict(encoder_outputs, attention_bias1, self.params.predict_inverse)

    def encode(self, inputs, attention_bias, predict_inverse=None):
