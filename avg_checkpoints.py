...
#flags.DEFINE_integer("first_num",10,'first_number')
#flags.DEFINE_integer("last_num",0,'last_number')
...
def main(_):
  if FLAGS.checkpoints:
    ...
    assert FLAGS.prefix, ("Prefix must be provided when averaging last"
                          " N checkpoints")
    tf.logging.error( FLAGS.prefix )
    tf.logging.error( os.path.dirname(FLAGS.prefix) )
    checkpoint_state = tf.train.get_checkpoint_state(
        os.path.dirname(FLAGS.prefix))
    print(checkpoint_state,FLAGS.prefix)
    checkpoints = checkpoint_state.all_model_checkpoint_paths[-FLAGS.num_last_checkpoints:]
    #checkpoints = checkpoint_state.all_model_checkpoint_paths[-FLAGS.num_last_checkpoints-12:-12]
  checkpoints = [c for c in checkpoints if checkpoint_exists(c)]
  tf.logging.info(checkpoints)
  if not checkpoints:
    if FLAGS.checkpoints:
      raise ValueError(
